...
namespace MeshBuilderLib
{
    ...
    {
        public static int[] Triangulate(Polygon polygon, bool flipFaceDirection = false)
        {
            ...
            for (int v = nv - 1; nv > 2;)
            {
                ...
                if (Snip(polygon.Points, u, v, w, nv, V))
                {
                    int a, b, c, s, t;
                    a = V[u];
                    b = V[v];
            ...
            if (!flipFaceDirection) indices.Reverse();
            return indices.ToArray();
        }
